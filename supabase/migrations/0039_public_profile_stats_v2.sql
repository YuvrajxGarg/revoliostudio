-- 0039_public_profile_stats_v2.sql
-- Revolio Studio — extends get_public_profile_stats (0038) with a
-- category breakdown (image/video/3d), peak activity hour, and per-day
-- category counts so the profile page's contribution graph can be
-- filtered by category. Same privacy stance as 0038: aggregate counts
-- only, never prompts/images/exact timestamps.

-- Return columns changed, so CREATE OR REPLACE isn't enough — Postgres
-- requires the old signature dropped first.
...
as $$
  ...
  select
    ...
    (select h from hour_counts) as peak_hour_utc,
    coalesce(
      (
