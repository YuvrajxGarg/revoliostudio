-- 0041_public_profile_stats_fav_count.sql
-- Revolio Studio — adds favorite_model_count to get_public_profile_stats
-- (0040) so the profile's "Favorite model" tile can show what share of a
-- user's work that model accounts for, rather than just naming it. Same
-- privacy stance as 0038-0040: aggregate counts only.
...
as $$
  ...
  select
    ...
    coalesce((select cnt from model_counts), 0)::bigint as favorite_model_count,
